...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = DeepFillGenerator(input_nc, output_nc)
    else:
...
class DeepFillGenerator(BaseNetwork):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        # stage2
        self.xconv1 = gen_conv(input_nc, cnum, 5, 1)
        ...
        self.allconv17 = gen_conv(cnum//4, output_nc, 3, 1, activation=None)
    ...
    def forward(self, x, mask_ori, mask_add):
        # xin = x
        ...
        x = self.pmconv6(x)
        if self.use_cam:
            mask_a = F.avg_pool2d(mask_add, kernel_size=4, stride=4)
            f = x * mask_a
            mask_s = F.avg_pool2d(mask_ori, kernel_size=4, stride=4)
            # similar = self.cam_1(x, x, mask_s)
            similar = self.cam_1(f, x, mask_s)
        ...
        if self.return_feat:
            ...
            return x_stage2, [feat, feat_x2, feat_x4]
        else:
